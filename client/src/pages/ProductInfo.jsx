...
const ProductInfo = () => {
  ...
  if (loading) return <div className='min-h-screen'> <div className='h-8 w-8 rounded-full animate-ping bg-[#ffa45c] absolute top-1/2 -translate-x-1/2 -translate-y-1/2 left-1/2'></div></div>
  ...
  return (
    <div className='px-3 md:my-10 my-4 min-h-screen'>
     ...
     <div className="fixed top-[13%] right-[3%] z-10 border rounded-full w-12 h-12 flex justify-center items-center bg-slate-100 cursor-pointer">
       <IoIosShareAlt className='text-slate-500' onClick={()=> {
     ...
     {copied && (
      ...
      <div className='flex justify-center max-w-4xl mx-auto '>
        {productInfo ? (
          <div key={productInfo._id} className='flex flex-col items-center text-left md:flex-row gap-4'>
            <div className='flex flex-col w-[500px]'>
              {/* Main Image */}
              <img src={mainImage} alt={productInfo.name} className='w-full h-72  object-contain' />
              
              {/* Thumbnails */}
              <div className='flex justify-center gap-2'>
                {productInfo.imageUrls.length > 1 &&  productInfo.imageUrls.map((url, index) => (
            ...
            <div className='flex flex-col sm:gap-3 gap-1'>
              <h1 className='md:text-3xl sm:font-semibold font-normal text-2xl'>{productInfo.name}</h1>
              ...
              {successmessage && (
                <p className='flex items-center justify-between bg-green-100 text-green-700 rounded-md p-2'>
                  ...
                  <LiaTimesSolid size={20} onClick={() => setSuccessMessage(null)} className='text-green-700 cursor-pointer' />
                </p>
              )}
