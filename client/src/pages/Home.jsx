...
// import hector from '../assests/hector.jpg'
...
const Home = () => {
  ...
  return (
    <main className='min-h-screen'>
      <div  style={{     background: `linear-gradient(to bottom, rgba(0, 0, 0, 1) 0%, rgba(0, 0, 0, 0.8) 60%, rgba(0, 0, 0, 0.3) 90%), url(${laptop}) center/cover no-repeat`, }}>
        <div className='w-full py-6 hidden sm:block px-3 '>
            <div className='max-w-6xl mx-auto flex md:gap-10 gap-2 justify-center '>
              ...
              <div className='py-2 flex-1'>
                  <p className='text-1xl py-3  text-[15px] space-x-10 text-gray-500'>NEW PRODUCT</p>
                ...
                <Link className='bg-[#ffa45c] text-white rounded-sm  p-3'>SEE PRODUCT</Link>
              </div>
       ...
       <div className='px-3 mt-6 mb-10'>
          <div className='max-w-6xl grid md:grid-cols-4 grid-cols-2 mx-auto gap-4'>
            {products.map((product) => {
              ...
              return (
                <Link to={`/product/${_id}`} key={_id}> {/* Corrected the Link path */}
                  <div className="bg-white shadow-md rounded-sm overflow-hidden flex flex-col">
                    <div className="relative w-full h-32 flex items-center justify-center">
                    <img src={imageUrls} alt={name} className="w-full h-32 mt-2 object-contain" />
                    </div>
                    <div className=" flex flex-col px-3 py-1 gap-1">
                      <h3 className=" md:font-semibold font-normal capitalize text-gray-600">{name}</h3>
                      <div className='flex items-center justify-between'>
                        <h3 className=" md:font-semibold font-normal  text-gray-600">${price}</h3>
                        <IoMdArrowDropright size={25} className='text-[#ffa45c]'/>                      
                      </div>
                    </div>
                  </div>
                </Link>
